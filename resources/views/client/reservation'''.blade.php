...
<section x-data="reservationForm({
  ...
  Alpine.data('reservationForm', ({ roomsData, extrasData }) => ({
    step: 1,
    startDate: '',
    endDate: '',
    adultsCount: 1,
    childrenCount: 0,
    selectedRooms: [], // Array pour stocker plusieurs IDs de chambres
    firstName: '',
    lastName: '',
    phone: '',
    typeId: '',
    ...
    init() {
      ...
      // Initialiser toutes les chambres comme disponibles au départ
      this.availableRooms = this.roomOptions;
    },
    
    // Dynamiser les chambres
    roomOptions: Array.isArray(roomsData) ? roomsData.map(room => ({
      id: room.id,
      name: room.categorie ? `Chambre ${room.categorie.typeChambre}` : `Chambre ${room.numero || 'Standard'}`,
      type: room.categorie ? room.categorie.typeChambre : 'Standard',
      price: room.prixNuit || 99,
      image: room.image ? `/images/${room.image}` : '/images/default-room.jpg',
      description: room.categorie ? room.categorie.description : 'Chambre confortable avec tous les équipements nécessaires pour votre séjour.',
    ...
    extraOptions: Array.isArray(extrasData) ? extrasData.map(extra => ({
        id: extra.id,
        name: extra.libelle || extra.nom, // S'adapte aux deux formats possibles
        price: extra.tarif || extra.prix, // S'adapte aux deux formats possibles
    ...
    toggleRoomSelection(roomId) {
      ...
      if (index === -1) {
        // Si la chambre n'est pas déjà sélectionnée, l'ajouter
        this.selectedRooms.push(roomId);
        console.log(`Chambre ${roomId} ajoutée à la sélection`);
      } else {
        // Si la chambre est déjà sélectionnée, la retirer
    ...
    // Calculer le prix total (pour multiples chambres)
    calculateTotalPrice() {
      ...
      this.extraOptions.forEach(extra => {
        if (extra.selected) {//on ne fait pas la différence , ceci est compté à partir de nombre de nuit passé
/*           if (extra.id === 1) {
            // Pour le petit-déjeuner ou tout service par personne/jour
            total += extra.price * (this.adultsCount + this.childrenCount) * nights;
          } else {
    ...
    /* validateStep2() {
      ...
      // Calculer la capacité totale de toutes les chambres sélectionnées
      this.selectedRooms.forEach(roomId => {
        const room = this.roomOptions.find(r => r.id === roomId);
        if (room) {
    ...
    /* checkAvailableRooms() {
      ...
      // Elle récupère la liste des chambres disponibles pour la période sélectionnée
      ...
      // Simulation - Doit être remplacée par un appel AJAX à votre endpoint
      fetch('/api/check-available-rooms', {
        method: 'POST',
        headers: {
      ...
      .then(data => {
        if (data.success) {
          ...
          this.availableRooms = this.roomOptions.filter(room => 
            data.availableRoomIds.includes(room.id)
          );
          
          // Réinitialiser la sélection si des chambres précédemment sélectionnées ne sont plus disponibles
          this.selectedRooms = this.selectedRooms.filter(id => 
            data.availableRoomIds.includes(id)
          );
          
          // Vérifier si nous avons assez de capacité pour le nombre de personnes
          const totalPeople = this.adultsCount + this.childrenCount;
          const totalAvailableCapacity = this.availableRooms.reduce((total, room) => total + room.capacity, 0);
          
          if (totalAvailableCapacity < totalPeople) {
            alert(`Attention: Il n'y a pas assez de chambres disponibles pour ${totalPeople} personnes. La capacité totale disponible est de ${totalAvailableCapacity} personnes.`);
          }
        } else {
          // Gérer l'erreur
          alert(data.message || 'Erreur lors de la vérification des chambres disponibles');
        }
      })
      .catch(error => {
    ...
    checkAvailableRooms(callback) {
  // Récupérer les chambres disponibles pour la période sélectionnée
      const data = {
        startDate: this.startDate,
        endDate: this.endDate,
        adultsCount: this.adultsCount,
      ...
      console.log('Vérification des chambres disponibles pour:', data);
      
      // Fallback en cas d'erreur avec l'API
      const handleFallback = () => {
        console.log('Utilisation du fallback pour les chambres disponibles');
        this.availableRooms = this.roomOptions;
        if (typeof callback === 'function') callback();
      };
      
      // Vérifier si la route API existe avant d'essayer de l'appeler
      ...
      const timeoutPromise = new Promise((_, reject) => 
        setTimeout(() => reject(new Error('Délai dépassé pour la vérification des chambres')), 5000)
      );
      ...
      .then(data => {
        ...
        // Appeler le callback après tout traitement
        if (typeof callback === 'function') callback();
      })
      .catch(error => {
        console.error('Erreur:', error);
    ...
    /* nextStep() {
      ...
      // Lorsqu'on passe de l'étape 1 à 2, filtrer les chambres disponibles
      if (this.step === 1) {
    ...
    nextStep() {
      ...
      if (this.step === 1) {
        ...
        // Essayer de vérifier les chambres disponibles, mais continuer même en cas d'échec
        try {
          this.checkAvailableRooms(() => {
            // Cette fonction callback sera exécutée quand la vérification est terminée
            this.step++;
            console.log('Passage à l\'étape:', this.step);
          });
        } catch (error) {
          console.error('Erreur lors de la vérification des chambres:', error);
          // En cas d'erreur, utiliser toutes les chambres comme disponibles
          this.availableRooms = this.roomOptions;
          this.step++;
          console.log('Passage à l\'étape (après erreur):', this.step);
        }
      }
    ...
    submitReservation() {
        ...
        // Supprimer toute entrée hidden existante pour éviter les doublons
        ...
        // Ajouter les IDs de toutes les chambres sélectionnées
        ...
        selectedServices.forEach((extra, index) => {
            ...
            extraInput.name = 'services[]'; // Format pour un tableau dans Laravel
            extraInput.value = extra.id;
            form.appendChild(extraInput);
            
            // Option: Ajouter également le nom et le prix pour référence (si nécessaire)
            /*
            const extraNameInput = document.createElement('input');
            extraNameInput.type = 'hidden';
            extraNameInput.name = `services_data[${index}][name]`;
            extraNameInput.value = extra.name;
            form.appendChild(extraNameInput);
            
            const extraPriceInput = document.createElement('input');
            extraPriceInput.type = 'hidden';
            extraPriceInput.name = `services_data[${index}][price]`;
            extraPriceInput.value = extra.price;
            form.appendChild(extraPriceInput);
            */
        });
        
        // Ajouter nombre d'adultes et d'enfants
  ...
  <div class="bg-white shadow-xl rounded-2xl p-6 md:p-8 max-w-4xl mx-auto">
    ...
    <!-- Étape 2: Sélection de chambre -->
      ...
      <div class="flex justify-between">
        ...
        <!-- Modifications à apporter à la section d'affichage des chambres (étape 2) -->

    <!-- Remplacer le bloc "Étape 2: Sélection de chambre" existant par ceci -->
    <div x-show="step === 2" class="fade-in" x-cloak>
        <h2 class="text-2xl font-bold text-[#6d4927] mb-6">Choisissez votre chambre</h2>
        ...
        <!-- Ajouter ces messages d'information -->
        <div x-show="availableRooms.length === 0" class="bg-yellow-50 border-l-4 border-yellow-400 p-4 mb-6">
        <div class="flex">
            ...
            <p class="text-sm text-yellow-700">
                Aucune chambre n'est disponible pour les dates sélectionnées. Veuillez choisir d'autres dates.
            </p>
            </div>
        </div>
        </div>
    
        <div x-show="adultsCount + childrenCount > availableRooms.reduce((total, room) => total + room.capacity, 0)" class="bg-yellow-50 border-l-4 border-yellow-400 p-4 mb-6">
        <div class="flex">
            <div class="flex-shrink-0">
            <svg class="h-5 w-5 text-yellow-400" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor">
                <path fill-rule="evenodd" d="M8.257 3.099c.765-1.36 2.722-1.36 3.486 0l5.58 9.92c.75 1.334-.213 2.98-1.742 2.98H4.42c-1.53 0-2.493-1.646-1.743-2.98l5.58-9.92zM11 13a1 1 0 11-2 0 1 1 0 012 0zm-1-8a1 1 0 00-1 1v3a1 1 0 002 0V6a1 1 0 00-1-1z" clip-rule="evenodd" />
            ...
            <div class="ml-3">
            <p class="text-sm text-yellow-700">
                Attention: La capacité totale des chambres disponibles (<span x-text="availableRooms.reduce((total, room) => total + room.capacity, 0)"></span> personnes) est inférieure au nombre de personnes sélectionné (<span x-text="adultsCount + childrenCount"></span>). Veuillez ajuster votre sélection.
            </p>
            </div>
        </div>
        </div>
    
        <!-- Remplacer la liste des chambres par la liste des chambres disponibles -->
        <div class="space-y-4 mb-12">
        ...
        <div x-show="availableRooms.length === 0" class="text-center py-12">
        <svg xmlns="http://www.w3.org/2000/svg" class="h-16 w-16 mx-auto text-gray-400" fill="none" viewBox="0 0 24 24" stroke="currentColor">
            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-3L13.732 4c-.77-1.333-2.694-1.333-3.464 0L3.34 16c-.77 1.333.192 3 1.732 3z" />
        </svg>
        <h3 class="text-lg font-medium text-gray-700 mt-4">Aucune chambre disponible pour cette période</h3>
        <p class="text-gray-500 mt-2">Veuillez essayer avec d'autres dates ou réduire la durée de votre séjour.</p>
        <button type="button" @click="step = 1" class="mt-4 px-4 py-2 bg-[#95714F] text-white rounded-lg hover:bg-[#6d4927] transition-colors duration-300">
            Modifier mes dates
        </button>
        </div>
