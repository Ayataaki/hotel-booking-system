...
{{-- <section class="py-16 px-4 max-w-5xl mx-auto" x-data="reservationForm({
  step: 1,
  startDate: '',
  endDate: '',
  adultsCount: 1,
  ...
  roomOptions: [
    {
      ...
      type: 'Standard',
      price: 99,
      image: 'https://picsum.photos/600/400?room=3',
    ...
    },
    {
      ...
      type: 'Deluxe',
      price: 129,
      image: 'https://picsum.photos/600/400?room=1',
    ...
    },
    {
      ...
      type: 'Prestige',
      price: 189,
      image: 'https://picsum.photos/600/400?room=2',
  ...
  selectedExtras: [],
  extraOptions: [
    { id: 1, name: 'Petit-déjeuner', price: 15, description: 'Petit-déjeuner continental avec produits frais', selected: false },
    { id: 2, name: 'Parking', price: 12, description: 'Place de parking sécurisée', selected: false },
    { id: 3, name: 'Accès Spa', price: 25, description: 'Accès illimité au spa pendant votre séjour', selected: false },
    { id: 4, name: 'Transfert aéroport', price: 40, description: 'Service de navette depuis/vers l\'aéroport', selected: false }
  ],
  ...
  calculateTotalPrice() {
    ...
    const room = this.roomOptions.find(r => r.id === this.selectedRoom);
    let total = room ? room.price * nights : 0;

    // Ajouter le prix des extras
    this.extraOptions.forEach(extra => {
      if (extra.selected) {
        // Pour les extras par personne comme le petit-déjeuner
        if (extra.id === 1) {
  ...
  validateStep2() {
    ...
    if (room && totalPeople > room.capacity) {
      alert(`La chambre sélectionnée ne peut accueillir que ${room.capacity} personnes`);
      return false;
    }
  ...
  validateStep4() {
    // La validation des détails de la carte est gérée par Stripe
  ...
  submitReservation() {
    ...
    // Cette partie sera remplacée par le code qui envoie les données au serveur
    alert('Traitement de la réservation...');

    // Simulation d'un envoi de formulaire (à remplacer par un véritable appel AJAX)
    setTimeout(() => {
...
<section x-data="reservationForm({
  ...
  extrasData: @json($services)
})">

{{-- <script>
  function reservationForm({ roomsData, extrasData }) {
    return {
      step: 1,
      startDate: '',
      endDate: '',
      ...
      roomOptions: roomsData.map(room => ({
        id: room.id,
        name: 'Chambre ' + (room.categorie?.typeChambre || 'Standard'),
        type: room.categorie?.typeChambre || 'Standard',
        price: room.prixNuit,
        image: room.image ? `/images/${room.image}` : '/images/default.jpg',
        description: room.categorie?.description || 'Description non disponible',
        capacity: room.capacite || 2
      })),

      // Dynamiser les services
      extraOptions: extrasData.map(extra => ({
        id: extra.id,
        name: extra.nom,
        price: extra.prix,
  ...
  submitReservation() {
    ...
    setTimeout(() => {
      ...
      // ... autres méthodes : nextStep, validateStep1, calculateTotalPrice, etc.
    };
  }
</script>  --}}
...
<script>
  ...
  return {
    ...
    submitReservation() {
      ...
      // Créer des entrées cachées pour les données sélectionnées
      const form = document.getElementById('reservationForm');
      
      // Ajouter l'ID de la chambre sélectionnée
      const roomInput = document.createElement('input');
      roomInput.type = 'hidden';
      roomInput.name = 'chambreId';
...
<form id="reservationForm" action="{{ route('reservation.store.online') }}" method="POST">
  ...
  <div class="bg-white shadow-xl rounded-2xl p-6 md:p-8">
      ...
      <div class="grid md:grid-cols-2 gap-6 mb-12">
        <div>
          ...
          <div class="flex items-center">
            <button @click="adultsCount > 1 ? adultsCount-- : adultsCount" class="w-10 h-10 bg-gray-100 rounded-l-lg flex items-center justify-center hover:bg-gray-200">
            ...
            <button @click="adultsCount < 10 ? adultsCount++ : adultsCount" class="w-10 h-10 bg-gray-100 rounded-r-lg flex items-center justify-center hover:bg-gray-200">
              <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4" viewBox="0 0 20 20" fill="currentColor">
        ...
        <div>
          ...
          <div class="flex items-center">
            <button @click="childrenCount > 0 ? childrenCount-- : childrenCount" class="w-10 h-10 bg-gray-100 rounded-l-lg flex items-center justify-center hover:bg-gray-200">
              <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4" viewBox="0 0 20 20" fill="currentColor">
                <path fill-rule="evenodd" d="M3 10a1 1 0 011-1h12a1 1 0 110 2H4a1 1 0 01-1-1z" clip-rule="evenodd" />
              </svg>
            </button>
            ...
            <button @click="childrenCount < 10 ? childrenCount++ : childrenCount" class="w-10 h-10 bg-gray-100 rounded-r-lg flex items-center justify-center hover:bg-gray-200">
              <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4" viewBox="0 0 20 20" fill="currentColor">
                <path fill-rule="evenodd" d="M10 5a1 1 0 011 1v3h3a1 1 0 110 2h-3v3a1 1 0 11-2 0v-3H6a1 1 0 110-2h3V6a1 1 0 011-1z" clip-rule="evenodd" />
              </svg>
            </button>
    ...
    {{-- <div x-show="step === 2" class="fade-in" x-cloak>
      ...
      <div x-data="{
        ...
        <div class="form-group">
            <label for="rooms">Choisissez des chambres :</label>
            <select id="rooms" x-model="selectedRooms" class="form-control" multiple>
    ...
    <!-- Section de sélection des chambres à ajouter dans la div x-show="step === 2" -->
<div class="space-y-4 mb-12">
  <template x-for="room in roomOptions" :key="room.id">
    <div class="border rounded-xl overflow-hidden cursor-pointer transition-all duration-300 hover:shadow-lg"
         :class="selectedRoom === room.id ? 'border-[#95714F] ring-2 ring-[#95714F] shadow-lg' : 'border-gray-200'"
...
<!-- Supprimez ou remplacez la div qui commence par x-data="{currentStep: 1..." car elle ne s'intègre pas correctement -->
    ...
    <div x-show="step === 4" class="fade-in" x-cloak>
      <div class="grid md:grid-cols-2 gap-8">
        ...
        <div class="order-2 md:order-1">
            ...
            <template x-if="selectedRoom">
              <div>
                <div class="flex justify-between items-start mb-4">
                  <div>
                    <h3 class="font-bold text-[#6d4927]" x-text="roomOptions.find(r => r.id === selectedRoom)?.name"></h3>
                    <p class="text-sm text-gray-600" x-text="roomOptions.find(r => r.id === selectedRoom)?.type"></p>
                  </div>
                  <img :src="roomOptions.find(r => r.id === selectedRoom)?.image" alt="Chambre" class="w-20 h-20 object-cover rounded-lg">
                </div>

                <div class="space-y-2 text-sm border-b border-gray-300 pb-4 mb-4">
                  <div class="flex justify-between">
                    <span>Arrivée:</span>
                    <span class="font-medium" x-text="new Date(startDate).toLocaleDateString('fr-FR', {day: 'numeric', month: 'long', year: 'numeric'})"></span>
                  </div>
                  <div class="flex justify-between">
                    <span>Départ:</span>
                    <span class="font-medium" x-text="new Date(endDate).toLocaleDateString('fr-FR', {day: 'numeric', month: 'long', year: 'numeric'})"></span>
                  </div>
                  <div class="flex justify-between">
                    <span>Durée:</span>
                ...
                <div class="space-y-2 text-sm border-b border-gray-300 pb-4 mb-4">
                  <div class="flex justify-between">
                    <span x-text="roomOptions.find(r => r.id === selectedRoom)?.name + ' (' + calculateNights() + ' nuit' + (calculateNights() > 1 ? 's' : '') + ')'"></span>
                    <span class="font-medium" x-text="formatPrice(roomOptions.find(r => r.id === selectedRoom)?.price * calculateNights())"></span>
                  </div>

                  <template x-for="extra in extraOptions.filter(e => e.selected)" :key="extra.id">
                    <div class="flex justify-between">
                      <span x-text="extra.name + (extra.id === 1 ? ' (' + (adultsCount + childrenCount) + ' personne' + ((adultsCount + childrenCount) > 1 ? 's' : '') + ' × ' + calculateNights() + ' jour' + (calculateNights() > 1 ? 's' : '') + ')' : '')"></span>
                      <span class="font-medium" x-text="formatPrice(extra.id === 1 ? extra.price * (adultsCount + childrenCount) * calculateNights() : extra.price)"></span>
                    </div>
                  </template>
                </div>
          ...
          <!-- Étape 4: Récapitulatif de la réservation - Section à corriger -->
          <div class="bg-[#F3ECE3] rounded-xl p-6 mb-6">
            <template x-if="selectedRoom">
              <div>
                <div class="flex justify-between items-start mb-4">
                ...
                <div class="space-y-2 text-sm border-b border-gray-300 pb-4 mb-4">
                  <div class="flex justify-between">
                    <span x-text="roomOptions.find(r => r.id === selectedRoom)?.name + ' (' + calculateNights() + ' nuit' + (calculateNights() > 1 ? 's' : '') + ')'"></span>
                    <span class="font-medium" x-text="formatPrice((roomOptions.find(r => r.id === selectedRoom)?.price || 0) * calculateNights())"></span>
                  </div>
            ...
            <template x-if="!selectedRoom">
              <div class="text-center py-4">
                <p class="text-gray-600">Veuillez sélectionner une chambre à l'étape 2.</p>
              </div>
            </template>
          </div>
        </div>
