...
{{--   rooms: [
  ...
  rooms: @json($chambres->map(function($chambre) {
      return [
      'id' => $chambre->id,
      /* 'name' => 'Chambre ' . ucfirst($chambre->categorie->nom ?? ''), */
      'type' => $chambre->categorie->nom ?? '',
      'description' => $chambre->categorie->description ?? 'Chambre confortable et accueillante',
      'price' => $chambre->prixNuit . '€/nuit',
      'numChambre' => $chambre->NumCh,
      'numEtage' => $chambre->NumEtg,
      'status' => $chambre->status,
      'capacite' => $chambre->capacite,
      'rating' => $chambre->categorie->notation ?? 3,
      'features' => $chambre->categorie->features ?? ['Wi-Fi gratuit', 'Climatisation', 'Bureau de travail'],
      'image' => $chambre->image ? asset('images/' . $chambre->image) : asset('images/chambre_standard_1.jpg'),
      'showDetails' => false
    ];
  },
  selectedRoom: null,
...
>
  ...
  >
    ...
    class="px-4 py-2 {{ request()->query('categorie') ? 'bg-white border border-[#95714F] text-[#95714F]' : 'bg-[#95714F] text-white' }} rounded-full hover:bg-[#95714F] hover:text-white transition-colors duration-300 focus:outline-none focus:ring-2 focus:ring-[#95714F] focus:ring-opacity-50 active:bg-[#6d4927]"
  >
    Toutes les chambres
  </a>
  @foreach($categories as $categorie)
  <a
    href="{{ route('chambres', ['categorie' => $categorie->id]) }}"
    class="px-4 py-2 {{ request()->query('categorie') == $categorie->id ? 'bg-[#95714F] text-white' : 'bg-white border border-gray-300 text-gray-700' }} rounded-full hover:border-[#95714F] hover:text-[#95714F] transition-colors duration-300 focus:outline-none"
  >
  ...
  <div class="text-center py-10">
    <p class="text-lg text-gray-600">Aucune chambre ne correspond à vos critères actuellement.</p>
  </div>
  @endif
  ...
  <div class="grid md:grid-cols-3 gap-12 max-w-[100%] card">
    <template x-for="(room, index) in rooms" :key="index">
      ...
      <div class="absolute top-2 left-2 z-10" x-show="room.status == '0'">
        <span class="bg-green-500 text-white text-xs px-2 py-1 rounded-full">
          Disponible
        </span>
      </div>
      <div class="absolute top-2 left-2 z-10" x-show="room.status === '1'">
...
{{--         <div class="p-6 flex flex-col h-[256px]">
 ...
 <div class="p-6 flex flex-col h-[256px]">
  ...
  <div class="text-sm text-gray-500 mb-1">
    <span x-text="'N° ' + room.numChambre + ' - Étage ' + room.numEtage"></span>
  </div>
...
{{--             <p class="text-sm text-gray-600" x-text="room.description"></p>
 --}}
            ...
            <p class="text-sm text-gray-600 mt-1">
              <span class="font-medium">Capacité:</span> <span x-text="room.capacite + ' personne(s)'"></span>
            </p>
 <!-- Caractéristiques (visibles uniquement quand showDetails est true) -->
          ...
          <div class="flex gap-2 mt-4">
            ...
            <a
              x-bind:href="'{{ route('reservation') }}?chambre=' + room.id"
              class="flex-1 text-center" 
              :class="room.status === 'disponible' ? 'bg-[#95714F] hover:bg-[#6d4927] text-white' : 'bg-gray-300 text-gray-500 cursor-not-allowed'"
              :disabled="room.status !== 'disponible'"
            >
              <span class="block font-medium py-2 px-2 rounded-lg transition-colors duration-300">
                Réserver
              </span>
            </a>
          </div>
      ...
      <div class="w-full md:w-1/2 p-6 overflow-y-auto max-w-[100%]">
        <template x-if="selectedRoom !== null">
          <div>
            ...
            <div class="mb-2">
              <span class="text-gray-600">
                <span class="font-medium">Chambre N°:</span> <span x-text="rooms[selectedRoom]?.numChambre"></span>
              </span>
              <span class="text-gray-600 ml-4">
                <span class="font-medium">Étage:</span> <span x-text="rooms[selectedRoom]?.numEtage"></span>
              </span>
            </div>
            
            <div class="mb-2">
              <span class="text-gray-600">
                <span class="font-medium">Capacité:</span> <span x-text="rooms[selectedRoom]?.capacite + ' personne(s)'"></span>
              </span>
              <span x-show="rooms[selectedRoom]?.status === 'disponible'" class="ml-4 text-green-600 font-medium">Disponible</span>
              <span x-show="rooms[selectedRoom]?.status === 'occupee'" class="ml-4 text-red-600 font-medium">Occupée</span>
            </div>
            ...
            <div class="flex space-x-4">
                <a
                x-bind:href="'{{ route('reservation') }}?chambre=' + rooms[selectedRoom]?.id"
                class="flex-1 text-center"
                :class="rooms[selectedRoom]?.status === 'disponible' ? 'bg-[#95714F] hover:bg-[#6d4927] text-white' : 'bg-gray-300 text-gray-500 cursor-not-allowed'"
              >
                <span class="block font-semibold py-3 rounded-lg transition-colors duration-300">
                  Réserver maintenant
                </span>
              </a>
              <button
